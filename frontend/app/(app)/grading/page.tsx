  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 mb-5">
        <p className="text-sm font-semibold text-gray-700 mb-2">10-Point Grading Scale</p>
      ...
      <div className="flex gap-3 mb-5">
        <select value={selectedOffering} onChange={(e) => { setSelectedOffering(e.target.value); setSelectedSheet(null); }}
          className="flex-1 max-w-md border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none">
              ...
              {!selectedSheet.is_locked && (
                <div className="flex gap-2">
                  ...
                  {selectedSheet.status === "draft" && (
                    <button onClick={() => submitSheet.mutate()} disabled={submitSheet.isPending}
                  ...
                  {selectedSheet.entries.map((e, i) => {
                    ...
                    return (
                      <tr key={e.student_id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
                        <td className="px-4 py-2.5 font-mono text-xs text-gray-600">{e.student_roll || "—"}</td>
                        ...
                        <td className="px-4 py-2.5">
                          {selectedSheet.is_locked ? <span>{e.internal_marks ?? "—"}</span> : (
                            <input type="number" min={0} max={40} value={m.internal} disabled={m.absent}
                              ...
                              className="w-20 border border-gray-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-[#0D6E6E] disabled:bg-gray-100" />
                          )}
                        </td>
                        <td className="px-4 py-2.5">
                        ...
                        <td className="px-4 py-2.5"><span className={`px-2 py-0.5 rounded text-xs font-bold ${e.grade_letter === "F" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>{e.grade_letter ?? "—"}</span></td>
            ...
            <div className="space-y-2">
              {selectedSheet.approvals.map((a) => (
                <div key={a.stage} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ${a.status === "approved" ? "bg-green-500" : a.status === "rejected" ? "bg-red-500" : "bg-gray-300"}`}>
                    {a.stage}
                  </div>
                  <div className="flex-1">
                ...
                {!otpSent ? (
                  ...
                  <div className="flex gap-2">
                    <input value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="Enter 6-digit OTP"
                      className="border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D6E6E] w-40" />
